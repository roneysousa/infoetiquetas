...
uses
  ...
  RpCon, RpConDS, rpBars, RpConBDE, Rpdevice;
type
  ...
  private
    ...
    Procedure Carregar_Dados(aDados : TClientDataSet);
...
begin
     ...
     begin
           Carregar_Dados(dmEtiquetas.cdsDados);
           RvProject1.Execute;
           RvProject1.Close;
     End;
     //
...
procedure TfrmPedidoEtiqueta.Carregar_Dados(aDados: TClientDataSet);
Var
  I, Quant : Integer;
begin
      //
                    ...
                    begin
                         With cdsEtiquetas do
                          begin
                               ...
                               FieldByName('VALOR').AsFloat   := aDados.FieldBYName('VALOR').AsFloat;
                               //
                               Post;
                          End;
                    End;
...
begin
  ...
  begin
   If (dmEtiquetas.tbParametrosPAR_IMPRESSORA.AsFloat = 2) Then
      //MarginTop   := 1.9  // Laser
      MarginTop   := dmEtiquetas.tbParametrosPAR_LINHA_INICIAL.AsFloat
   Else
      MarginTop   := dmEtiquetas.tbParametrosPAR_LINHA_INICIAL.AsFloat;
  ...
  Else
   ...
   Begin
     ...
     begin
       ...
       begin
         If (cdsEtiquetas.FieldByName('CODIGO').AsString <> '0000000000000') Then
         begin
       ...
       End;  
       //
       If (cdsEtiquetas.FieldByName('VALOR').AsFloat > 0) and (M_FLBARR) Then
       begin
            // Valor de venda
            RvSystem2.BaseReport.SetFont('Arial', fTamFonte);
            RVSystem2.BaseReport.GotoXY(CurCol,CurRow-0.04);
            RVSystem2.BaseReport.PrintRight( FormatFloat('###,##0.#0',cdsEtiquetas.FieldByName('VALOR').AsFloat),CurCol+3.2);
       End;
       // Descrição do produto
       RvSystem2.BaseReport.SetFont('Arial',6);
       ...
       If not Empty(cdsEtiquetas.FieldByName('CODIGO').AsString) and (M_FLBARR) Then
          PrintBarCodeEAN(RVSystem2,CurRow + 0.30,CurCol + 0.17, StrZero(FloattoStr(cdsEtiquetas.FieldByName('CODIGO').AsFloat),6))
       Else
       begin
            //ShowMessage('Coluna : ' +cdsEtiquetas.FieldByName('CODIGO').AsString + '  ' + cdsEtiquetas.FieldByName('VALOR').AsString );
            RvSystem2.BaseReport.SetFont('Arial Black',18);
            RVSystem2.BaseReport.GotoXY(CurCol ,CurRow + 0.9);  // + 0.03
            RVSystem2.BaseReport.PrintCenter(FormatFloat('###,##0.#0',cdsEtiquetas.FieldByName('VALOR').AsFloat),CurCol+1.8);  // 3.2
       End;
       //
     End;
     //
